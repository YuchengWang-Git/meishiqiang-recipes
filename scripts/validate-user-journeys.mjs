...
const terms = (item) => [item.name, item.canonicalName, ...(item.aliases || [])].filter(Boolean).map(normalize);
...
function byIngredients(selected) {
  return visible.map((recipe) => ({ recipe, matched: selected.filter((needle) => recipe.ingredients.some((item) => !pantryTerms.has(normalize(item.canonicalName || item.name)) && matches(item, needle))) }))
    .filter((entry) => entry.matched.length >= (selected.length === 1 ? 1 : 2));
}
...
assert(!combinationHits.some((entry) => entry.matched.length < 2), "多食材筛选仍会放入只匹配一种食材的菜谱");
...
console.log(`用户场景回归通过：${visible.length} 道可推荐菜谱；猪肉命中 ${porkHits.length} 道，甲鱼命中 ${turtleHits.length} 道，三食材组合命中 ${combinationHits.length} 道。`);
